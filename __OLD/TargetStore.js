	...
	init : (inner, self, params) => {
		...
		let saveTargetHash = (target) => {
			if (targetDataSet[target] === undefined) {
				...
				dataSet[target] = DSide.Store.generateHash(targetDataSet[target]);
			}
		...
		OVERRIDE(self.saveData, (origin) => {
			...
			saveData = self.saveData = (params) => {
				...
				if (validResult.isValid === true) {
					...
					// 데이터를 저장하기 전 검증합니다.
					if (data.createTime !== undefined && data.lastUpdateTime === undefined && DSide.Verify({
						signature : hash,
						address : address,
						data : data
					}) === true) {
						...
						// 1 토큰 소비
						if (isForSync === true || DSide.TokenStore.useToken({
							address : address,
							amount : 1
						}) === true) {
