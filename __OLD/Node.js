	...
	return {
		...
		init : (inner, self, params) => {
			...
			// 전체 데이터 관리자
			let dataManager = DSide.DataManager(dataStructures);
			...
			MULTI_PROTOCOL_SOCKET_SERVER({
				...
				on('getTokenStoreHash', (notUsing, ret) => {
					ret(DSide.TokenStore.getHash());
				});
				
				// 토큰 저장소의 계정들을 반환합니다.
				on('getTokenAccounts', (notUsing, ret) => {
					ret(DSide.TokenStore.getAccounts());
				});
				...
				on('getTokenBalance', (address, ret) => {
					...
					ret(DSide.TokenStore.getBalance(address));
				});
				...
				on('transferToken', (params, ret) => {
					...
					if (DSide.TokenStore.transfer(params) === true) {
			...
			let syncTokenStore = (fastestSend) => {
				...
				fastestSend('getTokenStoreHash', (hash) => {
					...
					if (DSide.TokenStore.getHash() !== hash) {
						...
						fastestSend('getTokenAccounts', (tokenAccounts) => {
							...
							EACH(tokenAccounts, (balance, accountAddress) => {
								...
								let _balance = DSide.TokenStore.getBalance(accountAddress);
								
								if (_balance !== balance) {
									
									DSide.TokenStore.increaseToken({
							...
							// 노드에 없는 계정이면 삭제
							EACH(DSide.TokenStore.getAccounts(), (balance, accountAddress) => {
								if (tokenAccounts[accountAddress] === undefined) {
									DSide.TokenStore.removeAccount(accountAddress);
								}
							});
						});
					}
			...
			INTERVAL(1, RAR(() => {
				...
				if (nowCal.getHour() === 0 && nowCal.getMinute() === 0 && nowCal.getSecond() === 0) {
					...
					EACH(connectionTimes, (connectionTime, accountAddress) => {
						DSide.TokenStore.chargeNodeReward({
							address : accountAddress,
					...
					// 토큰 데이터 통합
					let tokenStoreHash = DSide.TokenStore.getHash();
					
					let hashWeights = {};
					hashWeights[tokenStoreHash] = DSide.TokenStore.getBalance(accountAddress);
					...
					(node, done) => {
						...
						node.send('getTokenStoreHash', (hash) => {
							...
							if (isDone !== true) {
								...
								hashWeights[hash] += DSide.TokenStore.getBalance(node.accountAddress);
					...
					EACH(dataStructures, (dataStructure, storeName) => {
						...
						let hashWeights = {};
						hashWeights[storeHash] = DSide.TokenStore.getBalance(accountAddress);
